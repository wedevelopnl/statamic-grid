...
{
    protected static $handle = 'statamic-grid';
    ...
    public function __construct()
    {
        ...
        $this->pendingData = app(PendingGridData::class);
    }
     ...
     * Grid data is stored in PendingGridData and saved after EntrySaved event.
     */
    public function process(mixed $data): mixed
    {
        ...
        if (is_array($data) && ! empty($data)) {
            if ($entryId) {
                $this->pendingData->store($entryId, $data);
            } elseif ($slug) {
                $this->pendingData->storeBySlug($slug, $data);
            }
        }

        // Return marker so preProcess can load from database
        return ['__entry_id' => $entryId, '__slug' => $slug];
    }
